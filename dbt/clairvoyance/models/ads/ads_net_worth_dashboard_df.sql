...
pivoted as (
    select
        ...
        nullif(round(sum(case when asset_class = 'ssb'    then market_value_sgd else 0 end), 2), 0) as ssb_sgd
    from base
...
with_carried_forward as (
    select
        ...
        stocks_sgd,
        crypto_sgd,
        last_value(cash_sgd   ignore nulls) over (order by snapshot_date rows between unbounded preceding and current row) as cash_sgd,
        last_value(cpf_oa_sgd ignore nulls) over (order by snapshot_date rows between unbounded preceding and current row) as cpf_oa_sgd,
        last_value(cpf_sa_sgd ignore nulls) over (order by snapshot_date rows between unbounded preceding and current row) as cpf_sa_sgd,
        last_value(cpf_ma_sgd ignore nulls) over (order by snapshot_date rows between unbounded preceding and current row) as cpf_ma_sgd,
        last_value(ssb_sgd    ignore nulls) over (order by snapshot_date rows between unbounded preceding and current row) as ssb_sgd
    from pivoted
),
